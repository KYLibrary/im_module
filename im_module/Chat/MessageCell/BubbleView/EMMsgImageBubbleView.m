                               ...
                               imgSize:(CGSize)aSize
{
    ...
    if (img) {
        ...
        if (isAutoDownloadThumbnail) {
            [self sd_setImageWithURL:[NSURL URLWithString:aRemotePath] placeholderImage:[UIImage imageNamed:@"msg_img_broken" bundleName:@"Chat.bundle" inBundleForClass:[self class]] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                //            if (error) {
                //                self.image = [UIImage imageNamed:@"msg_img_broken"];
                //            }
            }];
        } else {
            self.image = [UIImage imageNamed:@"msg_img_broken" bundleName:@"Chat.bundle" inBundleForClass:[self class]];
        }
    }
