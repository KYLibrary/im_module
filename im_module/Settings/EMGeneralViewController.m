...
- (void)saveLogToDocument {
    [[EMClient sharedClient] getLogFilesPathWithCompletion:^(NSString *aPath, EMError *aError) {
        ...
        toPath = [toPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.gz",dateString]];
        NSError *error = nil;
        [fm copyItemAtPath:aPath toPath:toPath error:&error];
        if (error) {
            [EMAlertController showErrorAlert:@"移动失败"];
        }else {
